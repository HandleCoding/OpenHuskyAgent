...
| 在线 Chatbot / 应用后端 | HTTP SSE `/api/chat`、API Key 鉴权、多用户 session、按 agent 过滤工具 |
...
外部应用请通过 **Chatbot SSE**（`POST /api/chat`）或消息 channel 接入。详见 [docs/integrators.zh-CN.md](docs/integrators.zh-CN.md)。
...
| 模块 | 职责 |
|------|------|
| `service/` | Spring Boot 入口、HTTP SSE chatbot、TUI WebSocket、channel adapters、auth、Actuator |
...
| 文档 | 读者 |
|------|------|
| [docs/integrators.zh-CN.md](docs/integrators.zh-CN.md) | 把 Husky 当后端部署：agent、allowlist、Chatbot SSE、生产清单 |
