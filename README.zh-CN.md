...
Husky 的核心思想是元数据驱动的通用 Agent 计算：服务启动一次，每次请求根据来源、用户身份、Channel、**Agent**、Session、可见工具、Skill、Memory Policy、Knowledge Source、MCP 可见性、Workspace/Checkpoint Storage Policy、审批策略和执行后端来决定本次运行行为。
...
| 在线 Chatbot / 应用后端 | HTTP SSE `/api/chat`、OpenAI 兼容 `/v1/*`、API Key 鉴权、多用户 session、按 agent 过滤工具 |
| 企业助手 | 飞书、Telegram 和 Slack 多实例 bot、agent–channel binding、知识库、审计标签 |
| Agent 平台 | 分层 Java 模块、ToolProvider、Agent、Hook、RuntimePolicy、MCP |
...
| `X-Agent` | 否 | HTTP chatbot 请求显式指定 agent id |

SSE events 包括 `token`、`reasoning`、`message`、`tool_started`、`tool_completed`、`tool_failed`、`done` 和 `error`。

对开启限流的 agent，会返回带 rate-limit 语义的结构化错误，而不是静默丢弃请求。

### OpenAI 兼容 API（在 Husky 上搭应用）

把每个已配置 agent 暴露成 OpenAI 风格的 model，方便用标准 SDK 接入。

1. 开启配置：`openai-compatible.enabled=true` 或 `OPENAI_COMPATIBLE_ENABLED=true`。
2. 列出 models（即 agent id）：
...
3. Chat completions（支持 stream）。`model` 字段是 **agent id**（可用 `openai-compatible.model-prefix` 加前缀）：
...
在 `AUTH_ENABLED=true` 时与 Chatbot 共用 API Key。这是对外产品集成的推荐入口。配置、allowlist 与生产清单见 [docs/integrators.zh-CN.md](docs/integrators.zh-CN.md)。

### 飞书 Channel

Husky 支持飞书 WebSocket/webhook adapter，并支持 `channels.feishu.instances.*` 下的多 app 实例。每个实例可通过 `agent-channel-bindings.*` 绑定到不同 agent，从而暴露不同 prompt、tools、memory 和 approval policy。

### Telegram Channel

Husky 支持默认关闭的 Telegram long-polling adapter，并支持 `channels.telegram.instances.*` 下的多 bot 实例。配置 `TELEGRAM_ASSISTANT_ENABLED=true`、`TELEGRAM_ASSISTANT_BOT_TOKEN`，并建议配置 `TELEGRAM_ASSISTANT_BOT_USERNAME`，这样 `agent-channel-bindings.*` 才能把 bot 路由到指定 agent。
...
Husky 支持默认关闭的 Slack Socket Mode adapter，并支持 `channels.slack.instances.*` 下的多 bot 实例。配置 `SLACK_ASSISTANT_ENABLED=true`、`SLACK_ASSISTANT_BOT_TOKEN`、`SLACK_ASSISTANT_APP_TOKEN` 和 `SLACK_ASSISTANT_BOT_USER_ID` 后，`agent-channel-bindings.*` 才能把 bot 路由到指定 agent。启用 Slack 时 bot user id 是必填项，路由会 fail closed，不会静默落到全局默认。
...
- **Agent Runtime Policy** — agent 控制 prompt、toolsets、allow/deny、MCP、knowledge、skills、approval、backend、working directory、memory、audit、rate limit，以及可选 LLM model/provider。
- **Fail-closed allowlist** — 能力列表为空 = 无权限；`["*"]` = 全部。无效的具体 id 会在启动时失败。
...
- **Knowledge Layer** — 基于配置知识源暴露 agent-scoped `knowledge_search` 和 `knowledge_fetch`。
- **Skill System** — 内置 skills、用户安装 skills、SkillHub search/install，以及渐进式 `skill_list` / `skill_view` 加载。
- **MCP Integration** — 支持 stdio、SSE 和 Streamable-HTTP MCP servers，并由 agent 控制可见性。
- **Browser 和 Vision** — 启用后支持 Playwright browser automation 和本地/远程图片分析。
- **Sub-agent Delegation** — 匿名 `delegate_task` 子代理，三层策略（`agent.delegation` + `agents.*.delegation` + 工具参数）。
- **OpenAI 兼容面** — `/v1/models` 与 `/v1/chat/completions` 把 model 映射为 agent id，便于第三方客户端接入。
...
| `application/` | Agent 编排、channel adapters、agent/session resolution、runtime queues、JSON-RPC methods、sub-agent runner |
...
每个请求会解析 principal、channel identity、具体 account/bot binding、agent config 和 session id。生成的 runtime scope 决定 prompt sections、visible tools、skills、memory、knowledge sources、MCP servers、approval mode、execution backend、workspace/checkpoint storage、audit tags 和 graph cache keys。
...
| OpenAI 兼容 API | `openai-compatible.enabled`、`openai-compatible.model-prefix`、`openai-compatible.stream-timeout-ms` |
| Agents and bindings | `agents.*`、`agent-channel-bindings.*` — allowlist、`model`、`rate-limit-*`、`delegation` 见 [docs/integrators.zh-CN.md](docs/integrators.zh-CN.md) |
...
- 公网 `/api/chat` 与 `/v1/*` 保持 `AUTH_ENABLED=true`。
- 设置 `TUI_WS_ALLOWED_ORIGINS` 为可信 origin，不要用 wildcard `*` 暴露 TUI WebSocket。
- 把 `approval: none` 视为免审批模式；在线 agent 不要把它和危险 toolsets 组合使用。
- 公网入口优先使用能力收紧的专用 agent（例如 `chatbot`），不要直接暴露全能力 `assistant`。
- 面向公网的 agent 应限制 terminal、process、文件修改/搜索、browser automation、`skill_install`、`skill_manage` 和危险 MCP tools。
- 认真对待 empty / `["*"]` / 具体 allowlist；无效 skill、MCP、knowledge、toolset id 会在启动时 fail-closed。
- 多租户 HTTP 场景为 agent 开启 `rate-limit-*`。
...
| 文档 | 读者 |
|------|------|
| [docs/integrators.zh-CN.md](docs/integrators.zh-CN.md) | 把 Husky 当后端部署：agent、allowlist、OpenAI 兼容 API、生产清单 |
| [docs/agent_channel_binding_plan.md](docs/agent_channel_binding_plan.md) | `agents.*` / channels / bindings 配置模型 |
| [docs/agent-platform-roadmap.md](docs/agent-platform-roadmap.md) | 平台演进路线图 |
| [docs/roadmap.md](docs/roadmap.md) | 历史路线图索引 |
